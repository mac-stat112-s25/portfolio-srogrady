...
```{r}
# added a geom_bar() which created a bar graph visual on top of the frame
...
```{r}
# added labels that are more descriptive to compare two different categories
ggplot(hikes, aes(x = rating)) +
  geom_bar() +
...
```{r}
# Color coded the bar fillings to be blue making it stand out more
ggplot(hikes, aes(x = rating)) +
  geom_bar(fill = "blue") +
  labs(x = "Rating", y = "Number of hikes")
```
...
```{r}
# outlined the bar chunks with orange 
ggplot(hikes, aes(x = rating)) +
...
```{r}
# added " + theme_minimal()" to the code which appears to make the background theme white and more basic 
ggplot(hikes, aes(x = rating)) +
  geom_bar(color = "orange", fill = "blue")  +
  labs(x = "Rating", y = "Number of hikes") +
  theme_minimal()
...
when we are adding layers to the viz
-   We added the bars using `geom_bar()`? Why "geom"? 
geom because the bars are geometric objects to relay this data
-   What does `labs()` stand for?
labels
-   What's the difference between `color` and `fill`?
fill colors in the entire bar and color is the outline of the bar. 
#### Part b {.unnumbered}
...
1. Among the hikes, what's the *range* of elevation and how are the hikes *distributed* within this range (e.g. evenly, in clumps, "normally")?  3820-5344 distributed relatively normal
...
```{r}
# added the code geom_histogram() which layered a graph of the data (specifically a histogram visual) onto the frame
ggplot(hikes, aes(x = elevation)) +
  geom_histogram()
```

```{r}
# outlined the histograms bars in white 
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white") 
```

```{r}
# filled the data bars in with blue 
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white", fill = "blue") 
```

```{r}
# created good labels for the x and y axis 
ggplot(hikes, aes(x = elevation)) +
  geom_histogram(color = "white") +
...
```{r}
# changed the width of the bin that each hike falls into to 
ggplot(hikes, aes(x = elevation)) +
